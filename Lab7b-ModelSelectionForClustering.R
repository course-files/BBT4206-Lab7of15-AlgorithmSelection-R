# *****************************************************************************
# Lab 5: Model Selection for Clustering ----
#
# Course Code: BBT4206
...
# The data for each city was compiled by Inside Airbnb between
...
# Alternatively, we can decide to remove the 2 variables that have missing
# data
airbnb_cape_town_removed_var <-
...
dim(airbnb_cape_town_removed_var)

# Are there missing values in the dataset?
any_na(airbnb_cape_town_removed_var)

# OPTION 3:
# We can perform data imputation

# CAUTION:
# Avoid Over-imputation:
# Be cautious when imputing dates, especially if missingness is not random.
...
# it is better to remove the observations that have the missing data (OPTION 1)
...
corrplot(cor(airbnb_cape_town_removed_obs[, c(3, 6, 7, 9, 10, 11, 13, 14,
                                              15, 16)]), method = "square")

# Next, we identify the correlated variables. It is these correlated variables
...
# A scatter plot to show the price against the number of reviews
# per room type
...
airbnb_cape_town_removed_obs_norm <- predict(model_of_the_transform, # nolint
                                             airbnb_cape_town_removed_obs)
summary(airbnb_cape_town_removed_obs_norm)
# ##### DATA TRANSFORMATION

# k-means is sensitive to variables that have incomparable units, leading to
# misleading results. In this example, the number of reviews is tens of
# hundreds, but the price is tens of thousands.

# Without any data processing, the differences in price would appear to be
# larger than differences in reviews, but we want these variables to be treated
# equally.

# To avoid this problem, the variables need to be transformed to be on a
# similar scale. In this way, they can be compared correctly using the distance
# metric.

airbnb_cape_town[, c("price", "number_of_reviews")] = scale(airbnb_cape_town[, c("price", "number_of_reviews")])

# OPTION 1: Use only the most significant variables to create the clusters
airbnb_cape_town_2cols <-
  airbnb_cape_town_removed_obs[, c("price", "number_of_reviews")]

# OPTION 2: Use all the numeric variables to create the clusters
airbnb_cape_town_2cols <-
  airbnb_cape_town_removed_obs[, c(3, 6, 7, 9, 10, 11, 13, 14,
                                   15, 16)]

set.seed(7)
kmeans_cluster <- kmeans(airbnb_cape_town_2cols, centers = 3, nstart = 20)
...
for (i in 1:n_clusters) {
  ...
  kmeans_cluster <- kmeans(airbnb_cape_town_2cols, centers = i, nstart = 20)
...
scree_plot +
  geom_hline(
    yintercept = wss,
    linetype = "dashed",
    col = c(rep("#000000", 2), "#FF0000", rep("#000000", 5))
  )
...
kmeans_cluster <- kmeans(airbnb_cape_town_2cols, centers = k, nstart = 20)
...
ggplot(airbnb_cape_town_removed_obs, aes(airbnb_cape_town_removed_obs$calculated_host_listings_count, price,
                                         color = cluster_id)) +
  geom_point(alpha = 0.25) +
  xlab("Number of Reviews") +
  ylab("Price")

# Clustering for Descriptive Data Analytics and for Prescriptive Data Analytics ----
...
# to a supervised learning algorithm. The train model can then be used to
